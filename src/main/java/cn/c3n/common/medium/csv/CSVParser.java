...
import static cn.c3n.common.medium.csv.Token.Type.TOKEN;
...
public final class CSVParser implements Iterable<CSVRecord>, Closeable {
    ...
    private Map<String, Integer> initializeHeader() throws IOException {
        ...
        Map<String, Integer> hdrSameNameMap = new HashMap<String, Integer>();
        ...
        if (formatHeader != null) {
            ...
            if (headerRecord != null) {
                for (int i = 0; i < headerRecord.length; i++) {
                    ...
                    /////////////rename same column when needed
//                    if(header.equals("aLATACC"))
//                    {
//                        System.out.println(header);
//                    }

                    if(containsHeader && (!emptyHeader || !this.format.getAllowMissingColumnNames()) && this.format.getAllowSameName())
                    {
                        //build same header ruler
                        if(hdrSameNameMap.containsKey(header))
                        {
                            hdrSameNameMap.put(header, hdrSameNameMap.get(header) + 1);
                        }
                        else
                        {
                            hdrSameNameMap.put(header, 2);
                        }
                        header = header+"@"+hdrSameNameMap.get(header);
                    }

                    //renamed then try once again
                    containsHeader = hdrMap.containsKey(header);
    ...
    CSVRecord nextRecord() throws IOException {
        ...
        lineNumber++;
        boolean useCommentOnce = false;
        do {
            this.reusableToken.reset();
            //raw csv file row line number.
            boolean needSetComment = false;
            if(!useCommentOnce && this.format.getSkipLineNumbers().contains(lineNumber))
            {
                needSetComment = true;
                useCommentOnce = true;//use in this while once
            }

            this.lexer.nextToken(this.reusableToken, needSetComment);
            ...
            case COMMENT: // Ignored currently
                ...
                lineNumber++;//increse read a line
                useCommentOnce = false;//keep the ability to skip another comment
                break;
            default:
